...
_p.setStrokeWidth = function (nStrokeWidth) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        var sw = nStrokeWidth * this._getScaleFactor();
        if (BiBrowserCheck.versionNumber >= 6) sw += "px";
        tick.strokeweight = sw;
    }
};
...
_p._resizeLabels = function () {
    ...
    for (var i = 0; i < this._labels.length; i++) {
        ...
        } else this._labels[i].label.style.left = Math.round(this._labels[i].x - w); if (!this._labels[i]._topIsSet) {
...
function BiGauge2ThermometerNeedle() {
    if (_biInPrototype) return;
    BiAbstractGauge2ValueMarker.call(this);
};
_p = _biExtend(BiGauge2ThermometerNeedle, BiAbstractGauge2ValueMarker, "BiGauge2ThermometerNeedle");
BiGauge2ThermometerNeedle.addProperty("angle", BiAccessType.READ_WRITE);
BiGauge2ThermometerNeedle.addProperty("needlePosition", BiAccessType.READ_WRITE);
BiGauge2ThermometerNeedle.addProperty("needleBase", BiAccessType.READ_WRITE);
BiGauge2ThermometerNeedle.addProperty("needleHeight", BiAccessType.READ_WRITE);
BiGauge2ThermometerNeedle.addProperty("needleWidth", BiAccessType.READ_WRITE);
BiGauge2ThermometerNeedle.startValue = 890;
_p._tagName = "v:polyline";
_p.setValue = function (n) {
    this._value = n;
    if (this.getCreated()) this._calculatePoints();
...
_p._calculatePoints = function () {
    ...
    p3x = cx + p3x;
    p3y = cy - p3y;
    var needleWidth = this.getNeedleWidth() ? this.getNeedleWidth() : 50;
    var pointsValue1 = p1x + "," + Math.round(p1y) + " " + (p1x + needleWidth) + "," + Math.round(p1y) + " " + (p1x + needleWidth) + "," + (BiGauge2ThermometerNeedle.startValue - 50) + " " + p1x + "," + (BiGauge2ThermometerNeedle.startValue - 50) + " " + p1x + "," + Math.round(p1y);
    var pointsValue = p3x + "," + p3y + " " + p1x + "," + p1y + " " + p2x + "," + p2y + " " + p3x + "," + p3y;
    try {
        this._element.points.value = pointsValue1;
    } catch (ex) {
        this.setHtmlProperty("points", pointsValue1);
    }
};
_p.layoutComponent = function () {
    if (this._value == null) {
...
function BiGauge2ThermometerBulb() {
    if (_biInPrototype) return;
    BiGauge2Component.call(this);
};
_p = _biExtend(BiGauge2ThermometerBulb, BiGauge2Component, "BiGauge2ThermometerBulb");
_p._tagName = "v:oval";

function BiGauge2ThermometerPipe() {
    if (_biInPrototype) return;
    BiGauge2Component.call(this);
};
_p = _biExtend(BiGauge2ThermometerPipe, BiGauge2Component, "BiGauge2ThermometerPipe");
BiGauge2ThermometerPipe.addProperty("width", BiAccessType.READ_WRITE);
BiGauge2ThermometerPipe.addProperty("height", BiAccessType.READ_WRITE);
BiGauge2ThermometerPipe.addProperty("top", BiAccessType.READ_WRITE);
BiGauge2ThermometerPipe.addProperty("centerX", BiAccessType.READ_WRITE);
BiGauge2ThermometerPipe.addProperty("stroke", BiAccessType.READ_WRITE);
BiGauge2ThermometerPipe.addProperty("strokeWidth", BiAccessType.READ_WRITE);
_p._tagName = "v:polyline";
_p.layoutComponent = function () {
    var pointsValue1 = this._centerX + "," + this._height + " " + (this._centerX + this._width) + "," + this._height + " " + (this._centerX + this._width) + "," + 0 + " " + this._centerX + "," + 0 + " " + this._centerX + "," + this._height;
    try {
        this._element.points.value = pointsValue1;
    } catch (ex) {
        this.setHtmlProperty("points", pointsValue1);
    }
    this.setHtmlProperty("stroked", "True");
    this.setHtmlProperty("strokecolor", this._stroke);
    this.setHtmlProperty("strokeweight", this._strokeWidth - 3);
...
_p.setFont = function (o) {
    ...
    if (o instanceof BiFont) {
        ...
        else this.setStyleProperty("fontWeight", "normal"); if (!o.getSize) o.setSize(50);
